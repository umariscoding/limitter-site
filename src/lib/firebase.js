...
export const signUp = async (email, password, metadata = {}) => {
  try {
    ...
    try {
      ...
      toast.error(
        "Account created but couldn't send verification email. Please try signing in to resend it.",
      );
    }
  ...
  try {
    ...
    if (!userCredential.user.emailVerified) {
      ...
      try {
        ...
        throw new Error(
          "Please verify your email first. If you need a new verification email, try signing in again.",
        );
      }
    }
    ...
    if (existingDoc.exists()) {
      ...
      if (!existingData.is_active) {
        ...
        return {
          ...
          wasReactivated: true,
          message:
            "Website already added in the past. Reactivated with preserved settings.",
        };
      } else {
  ...
  try {
    ...
    if (planPrice > 0) {
      try {
        ...
        await updateUserStats({ plan: previousPlan }, "decrement");
        await updateUserStats({ plan: plan }, "increment");
        if (previousPlan !== plan) {
          await deleteAllUserSites(
            userId,
            `Plan changed from ${previousPlan} to ${plan}`,
          );
        }
    ...
    if (existingSubscription) {
      ...
      await grantPlanBenefits(
        userId,
        plan,
        previousPlan,
        "Website plan upgrade",
      );
      ...
      await updateDoc(doc(db, "user_overrides", userId), {
        ...
        overrides:
          data.overrides !== undefined
            ? data.overrides
            : data.purchased_overrides || data.override_credits || 0,
        overrides_left:
          data.overrides_left !== undefined
            ? data.overrides_left
            : data.overrides,
        total_overrides_purchased:
          data.total_overrides_purchased || data.credits_purchased_total || 0,
...
export const purchaseOverrides = async (userId, quantity, paymentData) => {
  try {
    ...
    const newTotalPurchased =
      (overrideStats.total_overrides_purchased || 0) + quantity;
...
export const getUserStatistics = async (userId) => {
  try {
    ...
    const currentlyBlockedSites = blockedSites.filter(
      (site) => site.is_blocked === true,
    ).length;
    const currentlyUnblockedSites = blockedSites.filter(
      (site) => site.is_blocked !== true,
    ).length;

    const totalTimeSpent = blockedSites.reduce(
      (sum, site) => sum + (site.total_time_spent || 0),
      0,
    );
    const todayTimeSpent = blockedSites.reduce(
      (sum, site) => sum + (site.time_spent_today || 0),
      0,
    );
    const totalTimeSaved = userProfile?.total_time_saved || 0;

    const totalAccesses = blockedSites.reduce(
      (sum, site) => sum + (site.access_count || 0),
      0,
    );
    const averageTimePerSite =
      totalSites > 0 ? Math.round(totalTimeSpent / totalSites) : 0;

    const lastWeek = new Date();
    lastWeek.setDate(lastWeek.getDate() - 7);
    const recentActivity = blockedSites.filter(
      (site) =>
        site.last_accessed &&
        new Date(
          site.last_accessed.toDate
            ? site.last_accessed.toDate()
            : site.last_accessed,
        ) > lastWeek,
    ).length;
...
export const getDashboardData = async (userId) => {
  try {
    const [userProfile, blockedSites, subscription, transactions] =
      await Promise.all([
        getUserProfile(userId),
        getBlockedSites(userId),
        getUserSubscription(userId),
    ...
    const transactionSummary = {
      ...
      total_spent: transactions.reduce(
  ...
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      sitesStatus[data.url] = {
        ...
        timeSpentToday:
          data.last_reset_date === today ? data.time_spent_today : 0,
        isBlocked: isBlocked,
  ...
  try {
    ...
    transactions.forEach((txn) => {
      if (txn.status === "completed") {
        ...
        transactionStats.by_type[txn.type] =
          (transactionStats.by_type[txn.type] || 0) + 1;

        const month = new Date(txn.created_at.seconds * 1000)
          .toISOString()
          .slice(0, 7);
        transactionStats.spending_history[month] =
          (transactionStats.spending_history[month] || 0) + txn.amount;
      }
    });

    transactionStats.average_transaction =
      transactionStats.total_transactions > 0
        ? transactionStats.total_spent / transactionStats.total_transactions
        : 0;

    const [siteAnalytics, overrideStats, usageHistory, timeSpentAnalytics] =
      await Promise.all([
...
export const getSiteAnalytics = async (userId, historyDays = 7) => {
  try {
    ...
    const siteAnalytics = sites.map((site) => {
      ...
      const recentTimeSpent = Object.values(filteredDailyUsage).reduce(
        (sum, time) => sum + time,
        0,
      );
      const averageDailyTime =
        recentTimeSpent / Math.max(1, Object.keys(filteredDailyUsage).length);
...
export const calculateSiteEfficiency = (site) => {
  ...
  const efficiency = Math.max(
    0,
    ((site.time_limit - todayUsage) / site.time_limit) * 100,
  );
...
export const getUsageHistory = async (userId, historyDays = 7) => {
  try {
    ...
    const totalTime = Object.values(dailyTotals).reduce(
      (sum, time) => sum + time,
      0,
    );
    ...
    const firstHalfAvg =
      firstHalf.reduce((sum, date) => sum + (dailyTotals[date] || 0), 0) /
      Math.max(1, firstHalf.length);
    const secondHalfAvg =
      secondHalf.reduce((sum, date) => sum + (dailyTotals[date] || 0), 0) /
...
export const getTimeSpentAnalytics = async (userId, historyDays = 7) => {
  try {
    ...
    const totalLimits = sites.reduce(
      (sum, site) => sum + (site.time_limit || 0),
      0,
    );
    const totalSpentToday = sites.reduce(
      (sum, site) => sum + (site.time_spent_today || 0),
      0,
    );
    const totalLifetimeSpent = sites.reduce(
      (sum, site) => sum + (site.total_time_spent || 0),
      0,
    );
    ...
    const productivityScore =
      totalLimits > 0
        ? Math.max(0, ((totalLimits - totalSpentToday) / totalLimits) * 100)
        : 100;
      ...
      .map((site) => ({
        ...
        percentage:
          totalLifetimeSpent > 0
            ? (site.total_time_spent / totalLifetimeSpent) * 100
            : 0,
      }));
...
const calculateTimeSaved = (sites, historyDays) => {
  const totalTimeSpent = sites.reduce(
    (sum, site) => sum + (site.total_time_spent || 0),
    0,
  );
  const totalLimits =
    sites.reduce((sum, site) => sum + (site.time_limit || 0), 0) * historyDays;
...
const calculateBlockedSessions = (sites) => {
  return sites.reduce((count, site) => {
    const dailyUsage = site.daily_usage || {};
    const blockedDays = Object.values(dailyUsage).filter(
      (usage) => usage >= (site.time_limit || Infinity),
    ).length;
...
export const adminGrantOverrides = async (
  userId,
  quantity,
  ...
  try {
    ...
    if (overrideStatsDoc.exists()) {
      ...
      await updateDoc(overrideStatsRef, {
        ...
        total_overrides_purchased:
          (currentData.total_overrides_purchased || 0) + quantity,
        updated_at: serverTimestamp(),
        last_admin_grant: {
          quantity: quantity,
          reason: reason,
...
export const adminChangeUserPlan = async (
  userId,
  newPlan,
  ...
  try {
    ...
    if (previousPlan !== newPlan) {
      await deleteAllUserSites(
        userId,
        `Admin changed plan from ${previousPlan} to ${newPlan}`,
      );
    }
    ...
    await updateUserStats({ plan: previousPlan }, "decrement");
    await updateUserStats({ plan: newPlan }, "increment");
...
export const grantPlanBenefits = async (
  ...
  try {
    ...
    if (newPlan === "elite") {
      ...
      newPlan === "free" &&
      (previousPlan === "elite" || previousPlan === "pro")
    ) {
      resetToZero = true;
    }
...
export const adminGetCollection = async (collectionName) => {
  try {
    ...
    console.error(
      `❌ Admin error fetching collection ${collectionName}:`,
      error,
    );
    throw error;
  }
...
export const adminSearchUsers = async (searchTerm, existingUsers) => {
  try {
    ...
    if (
      ...
      (searchTerm.includes("@") || searchTerm.length > 20)
    ) {
      // Basic check for email or ID
...
export const getUserRecentActivity = async (userId) => {
  try {
    ...
    try {
      ...
      console.log(
        "No user_activity collection yet or index needed:",
    ...
    if (overrideDoc.exists()) {
      ...
      if (
        overrideData.last_admin_grant &&
        overrideData.last_admin_grant.granted_at
      ) {
        activities.push({
          type: "admin_grant",
      ...
      if (overrideData.monthly_stats) {
        Object.entries(overrideData.monthly_stats)
          .slice(-3)
          .forEach(([month, stats]) => {
            if (stats.overrides_used > 0) {
              activities.push({
                ...
                timestamp: {
                  seconds: new Date(month + "-01").getTime() / 1000,
                },
                icon: "⚡",
                color: "purple",
              });
    ...
    activities.sort((a, b) => {
      const aTime =
        a.timestamp?.seconds || a.timestamp?.getTime?.() / 1000 || 0;
      const bTime =
        b.timestamp?.seconds || b.timestamp?.getTime?.() / 1000 || 0;
      return bTime - aTime;
    });
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      if (type === "create") {
        ...
        if (change.plan) {
          stats.users.byPlan[change.plan] = Math.max(
            0,
            (stats.users.byPlan[change.plan] || 0) - 1,
          );
        }
      } else if (type === "increment" && change.plan) {
        stats.users.byPlan[change.plan] =
          (stats.users.byPlan[change.plan] || 0) + 1;
...
export const recalculateAllStats = async () => {
  try {
    ...
    transactionsSnap.forEach((doc) => {
      ...
      stats.transactions.byType[transaction.type] =
        (stats.transactions.byType[transaction.type] || 0) + 1;
      stats.transactions.byStatus[transaction.status] =
        (stats.transactions.byStatus[transaction.status] || 0) + 1;
      stats.transactions.byPaymentMethod[transaction.payment_method] =
        (stats.transactions.byPaymentMethod[transaction.payment_method] || 0) +
        1;
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (transactionDetails.payment_method) {
        stats.transactions.byPaymentMethod[transactionDetails.payment_method] =
          (stats.transactions.byPaymentMethod[
...
export const createTransaction = async (userId, transactionData) => {
  try {
    ...
    const metadata = {};
    if (transactionData.metadata?.quantity)
      metadata.quantity = transactionData.metadata.quantity;
    if (transactionData.metadata?.price_per_unit)
      metadata.price_per_unit = transactionData.metadata.price_per_unit;
    if (transactionData.metadata?.previous_plan)
      metadata.previous_plan = transactionData.metadata.previous_plan;
    if (transactionData.metadata?.new_plan)
      metadata.new_plan = transactionData.metadata.new_plan;
    ...
    if (transaction.status === "completed") {
      ...
      await updateRevenueStats(transaction.amount, revenueType, {
...
export const deleteAllUserSites = async (userId, reason = "Plan change") => {
  try {
    ...
    const sitesRef = collection(db, "blocked_sites");
    const sitesQuery = query(sitesRef, where("user_id", "==", userId));
    const sitesSnapshot = await getDocs(sitesQuery);
    ...
    await Promise.all(
      Array(sitesSnapshot.size)
        .fill()
        .map(() => updateSiteStats(null, "delete")),
    );
...
export const adminGetDocumentIds = async (collectionName, searchTerm) => {
  try {
    ...
    console.error(
      `❌ Admin error searching documents in ${collectionName}:`,
      error,
    );
    throw error;
  }
...
export const adminGetContactMessages = async (lastDoc = null, limit = 20) => {
  let q = query(
...
export const adminUpdateContactMessage = async (messageId, updateData) => {
...
export const adminDeleteContactMessage = async (messageId) => {
...
export const adminGetDocument = async (collectionName, documentId) => {
  try {
    ...
    console.error(
      `❌ Admin error getting document from ${collectionName}:`,
      error,
    );
    throw error;
  }
